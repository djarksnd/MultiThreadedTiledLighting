...
bool TiledRenderer::Create(ID3D11Device* argDevice, ID3D11DeviceContext* immediateContext,
						   unsigned int argScreenWidth, unsigned int argScreenHeight, unsigned int argNumSubSamples)
{
	device = argDevice;
	device->AddRef();

	deviceContexts[ImmediateContextIndex] = immediateContext;
	deviceContexts[ImmediateContextIndex]->AddRef();
	...
	{
		thread.run = true;
		thread.thread = std::thread(RenderingThreadProc,
									std::ref(thread),
									std::ref(*this));
...
TiledRenderer::~TiledRenderer()
{
	...
	for (auto& deviceContext : deviceContexts)
	{
		if (deviceContext)
		{
